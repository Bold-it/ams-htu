...
app.post('/api/login', async (req, res) => {
    try {
        ...
        // FAILSAFE: Also allow plain-text 'admin123' for the super_admin during this repair phase
        const isFailsafeMatch = (user.email === 'accreditationsystem@htu.edu.gh' && password === 'admin123');

        if (!match && !isFailsafeMatch) {
            return res.status(401).json({ error: 'Invalid email or password' });
        }
...
app.get('/api/accreditations', async (req, res) => {
    try {
        const userEmail = (req.headers['x-user-email'] || 'anonymous').trim().toLowerCase();
        
        // Robust user lookup (case-insensitive email)
        const [userRows] = await pool.query('SELECT role, department FROM users WHERE LOWER(email) = ?', [userEmail]);
        const user = userRows[0];
        ...
        // Filter by department for non-admins (robust case-insensitive check)
        if (user && user.role !== 'admin' && user.role !== 'super_admin' && user.department && user.department.trim()) {
            query += ' WHERE LOWER(a.department) LIKE LOWER(?)';
            queryParams.push(`%${user.department.trim()}%`);
        }
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        // Non-blocking email verification for the health check
        // Specifically restricted to 3 seconds for fast API response
        ...
        res.json({
            version: 22, // VERSION 22: USER MANAGEMENT UPDATE
            status: 'ok',
            env_file_exists: fs.existsSync(path.join(__dirname, '.env')),
            email_service: verification.success ? 'connected' : 'auth_failed',
            email_error: verification.error || null,
            diagnostics: {
                host: (process.env.EMAIL_HOST || "").trim(),
                user_full: (process.env.EMAIL_USER || "").trim(),
